...
void processOrder(const Order& order) {
    ...
    std::cout << "Total time for preparation: " << order.total_time.count()
        << " seconds" << std::endl;  // общее время
...
void handleClient(SOCKET client_socket) {
    ...
    char client_message[4096];  // Стандартный размер буфера для ввода вывода
    // сетевых операций (прием и отправка данных по
    // сети)
    int client_message_length =
        recv(client_socket, client_message, sizeof(client_message), 0);
    // Получение смс от клиента
    ...
    std::cout << std::endl;
    std::cout << "Received order from Client #" << current_client_id
        << std::endl;  // Инфа о полученном заказе
    ...
    std::string order_keyword;          // Переменная для хранения отдельных ключевых
    // слов заказа
    ...
    while (iss >> order_keyword) {
        ...
        else if (order_keyword == "ice" && iss >> order_keyword &&
            order_keyword == "cream") {
            // Товар из двух слов
            order_keywords.push_back("ice cream");
        }
        else if (order_keyword == "free" && iss >> order_keyword &&
            order_keyword == "potato") {
    ...
    if (!order_keywords.empty()) {
        ...
        std::string order_message =
            "Your order: " + order_list + "\n";  // Тут только ключевые слова
        // из строки клиента
        send(client_socket, order_message.c_str(), order_message.size(), 0);
        // Отправка смс клиенту
        // СЕРВЕР
        std::cout << "Order: " << order_list << std::endl;  // Тоже самое но на
        // сервере
...
// ВРЕМЯ!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        for (const auto& keyword : order_keywords) {
            ...
            // Прибавление времени каждого товара
        }
        ...
        std::string time_message =
            "Total time for preparation: " + std::to_string(total_time.count()) + " seconds\n";
        send(client_socket, time_message.c_str(), time_message.size(), 0);
        // Отправка смс клиенту
        // СЕРВЕР
        std::cout << "----------------------------" << std::endl;
        std::cout << "Total time for preparation: " << total_time.count() << " seconds"
            << std::endl;  // Тоже самое но на сервере
        ...
        // ГОУ ТО КЛИЕНТ
        std::string total_cost_message =
            "Total cost: " + total_cost_stream.str() + "$\n";
        send(client_socket, total_cost_message.c_str(), total_cost_message.size(), 0);
        // Отправочка по сокету клиенту
        ...
        std::string ready_message = "Your order is ready\n";
        send(client_socket, ready_message.c_str(), ready_message.size(), 0);
        // Отправка что все готово, после истечения времени
